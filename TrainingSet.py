...
def run_generate(hardware_config, runtime_config, meta_config, do_inference=False):
    ...
    if(not do_inference):
        ...
        if(True):
            #dont need to re-synthesis
            import os
            if(os.path.exists("syn/"+dse_node+".syn.v")):
                print("already exists, skipping syn ", dse_node)
            ...
            #(TODOS) runtime_config
            from Generate_SYN import config2dse_node, syn
            from ptpx import ptpx
            dse_node = config2dse_node(hardware_config, runtime_config)
            import os
            print(dse_node)
            if(not os.path.exists("syn/"+dse_node+".syn.v")):
                print(dse_node, "does not exist in the synthesis folder !")
                exit(0)
            ptpx(dse_node, runtime_config, meta_config)
            os.system("chmod 777 ptpx.sh")
            os.system("./ptpx.sh")
...
def benchmark(hardware_config, debug = True):
    ...
    #The actual data (use real weights from resnet say)
    NETWORK = "SEQ"
    ...
    r = runtime_config
    ro = r["Operation_Params"]
    runtime_config["MING"] = "%s_kx%d_ky%d_x%d_y%d_i%d_n%d_b%d_str%d_p%d_w%s_a%s_clk%d_network%s"%(r["Operation"], ro["KX"], ro["KY"], ro["X"], ro["Y"], ro["I"], ro["N"], ro["B"], ro["STRIDE"], ro["PADDING"], r["WEI_PREC"], r["ACT_PREC"], r["CORE_CLK"], r["NETWORK"] )
                                                                                            ...
                                                                                            for WINO_TY0 in TILINGS0["WINO_TY"]:
                                                                                                for WINO_PRE_WEIGHT0 in TILINGS0["WINO_PRE_WEIGHT"]:
                                                                                                    ...
                                                                                                    if(count_samples):
                                                                                                        ...
                                                                                                        runtime_config = benchmark(hardware_config)
                                                                                                        run_generate(hardware_config, runtime_config, meta_config, do_inference)
                                                                                    else:
                                                                                        if(count_samples):
                                                                                            kongjian += 1
                                                                                        else:
...
#Statistics
training_set_1(count_samples= True, do_inference = False)

#Synthesis
training_set_1(count_samples= False, do_inference = False)
