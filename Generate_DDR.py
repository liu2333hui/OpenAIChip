...
hardware_config = {
        ...
        "DDR_WIDTH": 128,
        "BURST": 4,
        "DDR_ACT_FORMAT": "DENSE", #FEED, DENSE, DENSE_INTER_PE, SPARSE
        "DDR_WEI_FORMAT": "DENSE", #FEED, DENSE, SPARSE
...
def create_dataflow(FIXED_LOOP_ORDER, MAPPING, DATAFLOW, hardware_config, config):
    ...
    f.write("    "*depth + 'wei_data_f = open(\'' +  benchmark+"/weights.txt'"  +  ',"w")' +  '\n')
    f.write("    "*depth + 'act_data_f = open(\'' +     benchmark+"/activation.txt'"  + ',"w")' +  '\n')
    ...
    "+"    "*depth + "                            #print('  ACT', pe_no, '\tB', b, '\tI', i, '\tIX', x+kx, '\tIY', y+ky, '\tTN', n , activation[b][i][x+kx][y+ky]) #b, i, x + kx, y + ky)\n\
    "+"    "*depth + "                            pe_no += 1\n\
    "+"    "*depth + "pe_no = 0\n\
    "+"    "*depth + "for x in range(xx, min(xx+TX, (INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0]  )):\n\
    "+"    "*depth + "    for y in range(yy, min(yy+TY, (INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1)//STRIDE[1])):\n\
    ...
    if(hardware_config["DDR_ACT_FORMAT"] == "FEED"):#We assume there is broadcast in this version
        if(hardware_config["ACT_BROADCAST"] == False):
            f.write("\n\
    "+"    "*depth + "for x in range(xx, min(xx+TX, (INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0]  )):\n\
    "+"    "*depth + "    for y in range(yy, min(yy+TY, (INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1)//STRIDE[1])):\n\
    "+"    "*depth + "        for kx in range(kkx, min(kkx + TKX, KERNEL_SIZE[0])):\n\
    ...
    "+"    "*depth + "                    for n in range(nn, min(nn+TN, OUT_CHANNELS)):\n\
    "+"    "*depth + "                        for b in range(bb, min(bb + INPUT_BATCH, TB)):\n\
    "+"    "*depth + "                            act_ddr_idx += activation_precision\n\
    "+"    "*depth + "                            if(act_ddr_idx >= DDR_WIDTH):\n\
    "+"    "*depth + "                                #print(hex(activation[b][i][x][y])[2:].zfill(activation_precision//4), end='\\n'), whe shall add \\n to the code below for the correct DDR span\n\
    "+"    "*depth + "                                act_data_f.write(hex(activation[b][i][x+kx][y+ky])[2:].zfill(activation_precision//4)+'"+ddr_line_break+"')\n\
    "+"    "*depth + "                                act_ddr_idx = 0\n\
    "+"    "*depth + "                            else:\n\
    "+"    "*depth + "                                #print(hex(activation[b][i][x][y])[2:].zfill(activation_precision//4),'\t',end='')\n\
    "+"    "*depth + "                                act_data_f.write(hex(activation[b][i][x+kx][y+ky])[2:].zfill(activation_precision//4))\n\
    "+"    "*depth + "act_data_f.write('"+ddr_data_break+"')\n")
        else:
            f.write("\n\
    "+"    "*depth + "for x in range(xx, min(xx+TX, (INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0]  )):\n\
    "+"    "*depth + "    for y in range(yy, min(yy+TY, (INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1)//STRIDE[1])):\n\
    ...
    if(hardware_config["DDR_WEI_FORMAT"] == "FEED"):
        if(hardware_config["WEI_BROADCAST"] == False):
            f.write("\n\
    "+"    "*depth + "for x in range(xx, min(xx+TX, (INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0]  )):\n\
    "+"    "*depth + "    for y in range(yy, min(yy+TY, (INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1)//STRIDE[1])):\n\
    "+"    "*depth + "        for kx in range(kkx, min(kkx + TKX, KERNEL_SIZE[0])):\n\
    ...
    "+"    "*depth + "                    for n in range(nn, min(nn+TN, OUT_CHANNELS)):\n\
    "+"    "*depth + "                        for b in range(bb, min(bb + INPUT_BATCH, TB)):\n\
    "+"    "*depth + "                            wei_ddr_idx += weight_precision\n\
    "+"    "*depth + "                            if(wei_ddr_idx >= DDR_WIDTH):\n\
    "+"    "*depth + "                                wei_data_f.write(hex(weight[n][i][kx][ky])[2:].zfill(weight_precision//4)+'"+ddr_line_break+"')\n\
    "+"    "*depth + "                                wei_ddr_idx = 0\n\
    "+"    "*depth + "                            else:\n\
    "+"    "*depth + "                                wei_data_f.write(hex(weight[n][i][kx][ky])[2:].zfill(weight_precision//4))\n\
    "+"    "*depth + "#wei_data_f.write('"+ddr_data_break+"')\n")
        else:
            f.write("\n\
    "+"    "*depth + "for kx in range(kkx, min(kkx + TKX, KERNEL_SIZE[0])):\n\
    "+"    "*depth + "    for ky in range(kky, min(kky + TKY, KERNEL_SIZE[1])):\n\
    ...
    if(hardware_config["DDR_WEI_FORMAT"] == "DENSE"):
        depth = 1
        for var in FIXED_LOOP_ORDER:
            #FIXED_LOOP_ORDER = ["B", "KX", "KY", "X", "Y", "N", "I",
            #            "nn", "yy", "xx", "ii"]
            if(var in ['ii', 'nn', 'I', 'N', 'KX', 'KY']):
                f.write("    "*depth + MAPPING[var]+"\n")
                depth += 1
        if(hardware_config["WEI_BROADCAST"] == False):
            f.write("\n\
    ...
    "+"    "*depth + "                            else:\n\
    "+"    "*depth + "                                wei_data_f.write(hex(weight[n][i][kx][ky])[2:].zfill(weight_precision//4))\n\
    "+"    "*depth + "#wei_data_f.write(str(cycle)+'\\n')\n")
        else:
            f.write("\n\
    "+"    "*depth + "for kx in range(kkx, min(kkx + TKX, KERNEL_SIZE[0])):\n\
    "+"    "*depth + "    for ky in range(kky, min(kky + TKY, KERNEL_SIZE[1])):\n\
    ...
    #TODOS major issues here..
    if(hardware_config["DDR_ACT_FORMAT"] == "DENSE"): 
        depth = 1
        for var in FIXED_LOOP_ORDER:
            #FIXED_LOOP_ORDER = ["B", "KX", "KY", "X", "Y", "N", "I",
            #            "nn", "yy", "xx", "ii"]
            if(var in ['ii', 'B', 'KX', 'KY', 'X', 'Y', 'I', 'xx', 'yy']):
                f.write("    "*depth + MAPPING[var]+"\n")
                depth += 1
        if(hardware_config["ACT_BROADCAST"] == False):
            f.write("\n\
    "+"    "*depth + "print(kkx, kky, xx+TX, yy+TY)\n\
    "+"    "*depth + "if((kkx < TX and kky < TY) or (yy+TY >= ((INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1) // STRIDE[1]  )) or (xx+TX >= ((INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0]  ))):\n\
        "+"    "*depth + "for x in range(xx, min(xx+TX, (INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0]  )):\n\
        "+"    "*depth + "    for y in range(yy, min(yy+TY, (INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1)//STRIDE[1])):\n\
        "+"    "*depth + "        for kx in range(kkx, min(kkx + TKX, KERNEL_SIZE[0])):\n\
        "+"    "*depth + "            for ky in range(kky, min(kky + TKY, KERNEL_SIZE[1])):\n\
        ...
        "+"    "*depth + "act_data_f.write('"+ddr_data_break+"')\n")
            ...
            f.write("\n\
    "+"    "*depth + "#act_data_f.write(str(kkx)+'\t'+ str(kky)+'\t'+ str(xx+TX)+'\t'+str(yy+TY)+'\\n')\n\
    "+"    "*depth + "if((kkx < TX and kky < TY) or ( kkx >= (KERNEL_SIZE[0]-TX) and kky >= (KERNEL_SIZE[1]-TY) and yy+TY >= ((INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1) // STRIDE[1]  ) and (xx+TX >= ((INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0])  ))\
                    or (kky >= TY and   yy+TY >= ((INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1) // STRIDE[1]  ) and (xx+TX < ((INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0])  )))\
                    or (kkx >= TX and   yy+TY < ((INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1) // STRIDE[1] ) and (xx+TX >= ((INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0]  ))\
                    ):\n\
        "+"    "*depth + "for x in range(xx, min(xx+TX, (INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0]  )):\n\
        "+"    "*depth + "    for y in range(yy, min(yy+TY, (INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1)//STRIDE[1])):\n\
        "+"    "*depth + "        for kx in range(kkx, min(kkx + TKX, KERNEL_SIZE[0])):\n\
    ...
    elif(hardware_config["DDR_ACT_FORMAT"] == "DENSE_INTER_PE"):
        depth = 1
        for var in FIXED_LOOP_ORDER:
            #FIXED_LOOP_ORDER = ["B", "KX", "KY", "X", "Y", "N", "I",
            #            "nn", "yy", "xx", "ii"]
            if(var in ['ii', 'B', 'KX', 'KY', 'X', 'Y', 'I', 'xx', 'yy']):
                ...
                depth += 1
        f.write("\n\
    "+"    "*depth + "if((kkx < TX and kky < TY)):\n\
        "+"    "*depth + "if(kkx == 0 and kky == 0):\n\
            "+"    "*depth + "for x in range(xx, min(xx+TX, (INPUT_X + PADDING[0]*2 - KERNEL_SIZE[0] + 1) // STRIDE[0]  )):\n\
            "+"    "*depth + "    for y in range(yy, min(yy+TY, (INPUT_Y + PADDING[1]*2 - KERNEL_SIZE[1] + 1)//STRIDE[1])):\n\
            "+"    "*depth + "        for i in range(ii, min(ii+TI, IN_CHANNELS)):\n\
            "+"    "*depth + "            for b in range(bb, min(bb + INPUT_BATCH, TB)):\n\
            "+"    "*depth + "                act_ddr_idx += activation_precision\n\
            "+"    "*depth + "                if(act_ddr_idx >= DDR_WIDTH):\n\
            "+"    "*depth + "                    act_data_f.write(hex(activation[b][i][x][y])[2:].zfill(activation_precision//4)+'"+ddr_line_break+"')\n\
            "+"    "*depth + "                    act_ddr_idx = 0\n\
            "+"    "*depth + "                else:\n\
            "+"    "*depth + "                    act_data_f.write(hex(activation[b][i][x][y])[2:].zfill(activation_precision//4))\n\
            "+"    "*depth + "act_data_f.write('"+ddr_data_break+"')\n")
...
#################################Create the hardware########################
#############we have already created the data files
#############1. create the macro configuration file
MACRO_CFG = {
    "CORE_CLK" : CORE_CLK,
    "DDR_CLK"  : DDR_CLK ,
    #tiling used for current layer
    "TX": TX,
    "TY": TY,
    "TKX": TKX,
    "TKY": TKY,
...
for k in MACRO_CFG:
    "`define\t" + k + "\t" + MACRO_CFG[k]+";"
    
#############2. Create the verilog
'''
#############3. create the testbench
module conv_tb;
reg core_clk, ddr_clk, rst_n;
