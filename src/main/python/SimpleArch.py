...
class SimpleArch:#(AIChip):
	...
	def __init__(self, config, model_name = "Testing", run_cpp = True, np_save = True,
		...
		self.log_root = f"generated/Architecture/SimpleArch/{logs}/Design_"+design + "__" + model_name + "__" + benchmark

		if not os.path.exists(f"generated/Architecture/SimpleArch/{logs}"):
		    os.mkdir(f"generated/Architecture/SimpleArch/{logs}")	
	...
	def infer_fc_in_buffers(self, name,
		...
		def fc_buffer_toggle(prec, num_data, bank_bits, tile,
			...
			if(self.run_cpp):
				print(f"g++ -O3 {TOGGLE_FILE}") #make this as fast as possible please
	...
	def infer_fc_mult(self, name,
		...
		PE_TRACE_FILE = PE_OUT_FILE+".trace.in"
		PE_TRACE_FILE_0 = PE_TRACE_FILE+"_0"
		PE_TRACE_FILE_1 = PE_TRACE_FILE+"_1"
		...
		if(self.RUN_PE):
			...
			if(self.RUN_MODEL):
				...
				with open(PE_OUT_FILE, "r") as f:
					...
					input_data = trace_file(num=2, TRACE_FILE=PE_TRACE_FILE, input_data=input_data, r = 1)
					#input_data['in_0'] = [[i] for i in input_data['in_0']
					print(input_data)
					input_data['in_0'] = [[element] for sublist in input_data['in_0'] for element in sublist]
					input_data['in_1'] = [[element] for sublist in input_data['in_1'] for element in sublist]
	
					#input data need to convert from
					#[0,1,2] to [[0],[1],[2]]

					#print(input_data)
					#(please debug, why cannot load the pickle?)
					res = mul.execute_testing(
						name = "Multiplier2",
		...
		input_data = {
            ...
            "radix": [4] 
					...
					#print(res)
					estimate_pwr = res['Total_Pwr']['res_avg']
					
					power_per_toggle, avg_pwr = mul.execute_get_lut(name="Multiplier2",out_features="Total_Pwr", constant_features = {
					 	"CLOCK": self.clock,
					 	"cap_load": self.cap_load,
					 	"prec1": self.WEI_PREC,
					 	"prec2": self.ACT_PREC,
						...
						variable_features = {
						"bits_in_0": [i for i in range(self.ACT_PREC)] ,
						"bits_in_1": [i for i in range(self.WEI_PREC)]
					})
	
					#power_per_toggle = mult_power
					avg_pwr = np.sum(power_per_toggle)/len(power_per_toggle)
					print("FC_PES", "avg_pwr")
					print(self.FC_PES, avg_pwr)
					print(res)

					baseline1_pwr = self.FC_PES * avg_pwr
					baseline2_pwr = baseline1_pwr#assume same for now

					PE_POWER = self.FC_PES*estimate_pwr#based on multiple-pe
					print("raw estimate pwr = ", estimate_pwr)
	...
	def infer_fc_adder_accum(self,name,
			IN, OUT, BAT, w_file, i_file, weights, input_data):
		...
		PE_OUT_FILE = self.root+"/"+name+f".{TYPE_NAME}.pe.out"
		PSUM_OUT_FILE = self.root+"/"+name+f".{TYPE_NAME}.psum.out"
		OUT_OUT_FILE = self.root+"/"+name+f".{TYPE_NAME}.out.out"
	
		PE_TRACE_FILE = self.root+"/"+name+f".{TYPE_NAME}.pe.trace"
		PSUM_TRACE_FILE = self.root+"/"+name+f".{TYPE_NAME}.psum.trace"
		OUT_TRACE_FILE = self.root+"/"+name+f".{TYPE_NAME}.out.trace"
	
	
		TOGGLE_FILE = self.root+"/"+name+f".{TYPE_NAME}.cpp"
		JSON_FILE = self.root+"/"+name+f".{TYPE_NAME}.json"
		POWER_GOLDEN_FILE = self.root+"/"+name+f".{TYPE_NAME}.golden"
		ADDER_TREE_POWER_GOLDEN_FILE = self.root+"/"+name+f".{TYPE_NAME}.adder_tree.golden"
		ACCUMULATOR_POWER_GOLDEN_FILE = self.root+"/"+name+f".{TYPE_NAME}.accumulator.golden"
		...
		with open(TOGGLE_FILE, "w") as f:
			...
			f.write(f'''
			int toggle_psum[{self.FC_PES//self.FC_TI}][{prec}];
			for (int k = 0; k < {self.FC_PES//self.FC_TI}; k++)
				for (int j = 0; j < {prec}; j++) {{
					toggle_psum[k][j] = 0;
			}}''')

			f.write(f'''
			int toggle_output[{self.FC_PES//self.FC_TI}][{prec}];
			for (int k = 0; k < {self.FC_PES//self.FC_TI}; k++)
				for (int j = 0; j < {prec}; j++) {{
					toggle_output[k][j] = 0;
			}}''')
			...
			if(True):
				f.write(f'''
					int prev_val[{self.FC_PES}];
					for(int k = 0; k < {self.FC_PES}; k++)
						prev_val[k] = 0;
				''')

				f.write(f'''
					int prev_psum[{self.FC_PES//self.FC_TI}];
					for(int k = 0; k < {self.FC_PES//self.FC_TI}; k++)
						prev_psum[k] = 0;
				''')

				f.write(f'''
					int prev_out[{self.FC_PES//self.FC_TI}];
					for(int k = 0; k < {self.FC_PES//self.FC_TI}; k++)
						prev_out[k] = 0;
				''')
				...
				f.write(f'std::cout << "// Analyzing Workload" << std::endl;\n')
				if(self.RUN_GOLDEN):
					for i in range(self.FC_TI):#terms = FC_TI for adder tree	
						f.write(f'std::ofstream goldenOutFile_PE_{i}("{PE_TRACE_FILE}_{i}");\n');
					for i in range(1): #terms = 1 for now
						f.write(f'std::ofstream goldenOutFile_PSUM_{i}("{PSUM_TRACE_FILE}_{i}");\n');
	
						f.write(f'std::ofstream goldenOutFile_OUT_{i}("{OUT_TRACE_FILE}_{i}");\n');
  ...
  #       return output
	...
	def infer_max_pool(self, name, input_data, out_data, Randomize = False):
		B, I, H, W = input_data.shape
		...
		results = []
		#if(self.RUN_L1 or self.RUN_L2 or self.RUN_WEI_BUFFERS or self.RUN_ACT_BUFFERS):
			#b_res = self.infer_fc_in_buffers(name, IN, OUT, BAT, w_file, i_file, weights, input_data, out_data)
			#results = results + b_res
	
		if(self.RUN_MAX_UNIT):
			m_res = self.infer_fc_mult(name, IN, OUT, BAT, w_file, i_file, weights, input_data)
			results = results + m_res	
		'''
		return results	
	...
	def infer_network(self,intermediate_outputs,skips=[]):
		for layer_name, layer, input_data, out_data in intermediate_outputs:
			...
			if isinstance(layer, nn.Conv2D):
				...
				#results = self.infer_max_pool(layer_name, input_data, out_data, Randomize=self.Randomize)
...
if __name__ == "__main__":
	...
	for tb in [1,2,4]:
		...
		for tn in [9,12, 16,32,64]:
			for ti in [ 2,8,4, 16,32,2]:	
	...
	for c in combos:#[combos[0]]):#[combos[0]]:
		...
		logs = "log_full_0321_50",

		SIM_CYCLES = 50, Randomize = True,
