...
class SystolicConv(GeneralConvUnit):
	...
	def get_primitive_statistics(self, hc):
		...
		#PE ARRAY
		MultiplierComponent(net, self, "PE_ARRAY", loop_order = self.hc["LOOP_ORDER"], units = TILE, bins = ['weights_data', 'inputs_data'], main = "weight", wei_prec = "WEI_PREC", act_prec = "ACT_PREC", data_obj = ["weights_obj", "inputs_obj"], runtime_lambda = lambda p : MAC_NO // p['avg_cycle_per_op'] )

		#AdderAccumulateComponent(net, self, "PE_ARRAY", loop_order = self.hc["LOOP_ORDER"], units = TILE, bins = ['weights_data', 'inputs_data'], main = "weight", wei_prec = "WEI_PREC", act_prec = "ACT_PREC", data_obj = ["weights_obj", "inputs_obj"], runtime_lambda = lambda p : MAC_NO // p['avg_cycle_per_op'] )

		AdderAccumulateComponent(ADDER_TYPE = "ADDER_TREE", valid_nets=net, self=self, unit_name="ADDER_TREE", loop_order = self.hc["LOOP_ORDER"], units =ADDER_TREE_UNITS, input_group = self.hc["TKX"]*self.hc["TKY"]*self.hc["TI"], bins = [], prec = "PREC", data_obj = ["weights_obj", "inputs_obj"], update = "weights_data*inputs_data", cast_skips = [], out_prec = "OUT_PREC")

		

		#MEMORY
		MemoryComponent(net, self,"L1_WEI_READ", loop_order=self.hc["LOOP_ORDER"],cast_skips=["TX", "TY", "TB"], units=TILE, bins = ['weights_data'], prec = "PREC", data_obj = ['weights_obj'], runtime_lambda = lambda p : SIZE // p['avg_cycle_per_op'], fanout = 1 , ratio = self.hc.get("LOAD_RATIO",1), loop_var = {}, CustomMap = {}, SRAM_SIZE = "L1_WEI_SRAM_SIZE", SRAM_TYPE = "L1_WEI_SRAM_TYPE", MODE = 0)	
		
		MemoryComponent(net, self,"L1_WEI_WRITE", loop_order=filter_loop_order(self.hc["LOOP_ORDER"], ["N", "KX", "KY", "I"])    ,cast_skips=[], units=L1_WEI_BUF_BIT_LEN, bins = ['weights_data'], prec = "PREC", data_obj = ['weights_obj'], runtime_lambda = lambda p : SIZE // p['avg_cycle_per_op'], fanout = 1 , ratio = self.hc.get("LOAD_RATIO",1), loop_var = {}, CustomMap = {}, SRAM_SIZE = "L1_WEI_SRAM_SIZE", SRAM_TYPE = "L1_WEI_SRAM_TYPE", MODE = 1)	
		...
		MemoryComponent(net, self,"L1_ACT_READ", loop_order=self.hc["LOOP_ORDER"],cast_skips=["TN"], units=TILE, bins = ['inputs_data'], prec = "PREC", data_obj = ['inputs_obj'], runtime_lambda = lambda p : SIZE // p['avg_cycle_per_op'], fanout = 1 , ratio = self.hc.get("LOAD_RATIO",1), loop_var = {}, CustomMap = {}, SRAM_SIZE = "L1_ACT_SRAM_SIZE", SRAM_TYPE = "L1_ACT_SRAM_TYPE", MODE = 0)	

		MemoryComponent(net, self,"L1_ACT_WRITE", loop_order=filter_loop_order(self.hc["LOOP_ORDER"], ["B","X", "Y", "KX", "KY", "I"])    ,cast_skips=[], units=L1_ACT_BUF_BIT_LEN, bins = ['inputs_data'], prec = "PREC", data_obj = ['inputs_obj'], runtime_lambda = lambda p : SIZE // p['avg_cycle_per_op'], fanout = 1 , ratio = self.hc.get("LOAD_RATIO",1), loop_var = {}, CustomMap = {}, SRAM_SIZE = "L1_ACT_SRAM_SIZE", SRAM_TYPE = "L1_ACT_SRAM_TYPE", MODE = 1)	



		MemoryComponent(net, self,"L1_OUT_READ", loop_order=filter_loop_order(self.hc["LOOP_ORDER"], ["X", "Y", "N", "B"]),cast_skips=[], units=L1_OUT_BUF_BIT_LEN, bins = ['outputs_data'], prec = "PREC", data_obj = ['outputs_obj'], runtime_lambda = lambda p : SIZE // p['avg_cycle_per_op'], fanout = 1 , ratio = self.hc.get("LOAD_RATIO",1), loop_var = {}, CustomMap = {}, SRAM_SIZE = "L1_OUT_SRAM_SIZE", SRAM_TYPE = "L1_OUT_SRAM_TYPE", MODE = 0)	

		MemoryComponent(net, self,"L1_OUT_WRITE", loop_order=filter_loop_order(self.hc["LOOP_ORDER"], ["X", "Y", "N", "B"]),cast_skips=[], units=L1_OUT_BUF_BIT_LEN, bins = ['outputs_data'], prec = "PREC", data_obj = ['outputs_obj'], runtime_lambda = lambda p : SIZE // p['avg_cycle_per_op'], fanout = 1 , ratio = self.hc.get("LOAD_RATIO",1), loop_var = {}, CustomMap = {}, SRAM_SIZE = "L1_OUT_SRAM_SIZE", SRAM_TYPE = "L1_OUT_SRAM_TYPE", MODE = 1)	


		#NETWORKS
		if("ACT_PE_CAST" in net):
			ratio = self.hc["CAST_RATIO"]
			NetworkComponent(valid_nets = net, self = self, unit_name = "ACT_PE_CAST", loop_order = self.hc["LOOP_ORDER"] , cast_skips = ["TN"], units = TILE, bins = ['inputs_data', f'{self.hc["TN"]}*std::min({self.hc["TKX"]} - std::abs(x % {self.hc["TX"]} - {self.hc["TKX"]} + 1) , {self.hc["TKX"]})* std::min({self.hc["TKY"]} - std::abs(y % {self.hc["TY"]} - {self.hc["TKY"]} + 1) , {self.hc["TKY"]})'], prec = "PREC", data_obj = ['inputs_obj'], runtime_lambda = lambda p : 1,  fanout = 1, ratio = ratio, CustomMap = {"in_1": "fanout"}) 
		...
		if("WEI_PE_CAST" in net):
			ratio = self.hc["CAST_RATIO"]
			NetworkComponent(valid_nets = net, self = self, unit_name = "WEI_PE_CAST", loop_order = self.hc["LOOP_ORDER"] , cast_skips = ["TB", "TX", "TY"], units = TILE, bins = ['weights_data'], prec = "PREC", data_obj = ['weights_obj'], runtime_lambda = lambda p : 1,  fanout = max(1, (self.hc["TB"]*self.hc["TX"]*self.hc["TY"])//ratio))
		...
		if("L1_ACT_READ_NETWORK" in net):
			...
			NetworkComponent(valid_nets = net, self = self, unit_name = "L1_ACT_READ_NETWORK", loop_order = self.hc["LOOP_ORDER"] , loop_var = LOOP_VAR_DEFINITIONS, cast_skips = ["TN"], units = TILE, bins = ['inputs_data'], prec = "PREC", data_obj = ['inputs_obj'], runtime_lambda = lambda p : 1,  fanout = 1, ratio = ratio) 
		...
		if("L1_WEI_READ_NETWORK" in net):
			...
			NetworkComponent(valid_nets = net, self = self, unit_name = "L1_WEI_READ_NETWORK", loop_order = self.hc["LOOP_ORDER"] , loop_var = LOOP_VAR_DEFINITIONS, cast_skips = ["TB", "TX", "TY"], units = TILE, bins = ['weights_data'], prec = "PREC", data_obj = ['weights_obj'], runtime_lambda = lambda p : 1,  fanout = 1, ratio = ratio) 
		...
		if("L1_OUT_WRITE_NETWORK" in net):
			...
			NetworkComponent(valid_nets = net, self = self, unit_name = "L1_OUT_WRITE_NETWORK", loop_order = filter_loop_order(self.hc["LOOP_ORDER"] , ["X", "Y", "N", "B"]), loop_var = LOOP_VAR_DEFINITIONS, cast_skips = [], units = L1_OUT_BUF_BIT_LEN, bins = ['outputs_data'], prec = "PREC", data_obj = ['outputs_obj'], runtime_lambda = lambda p : 1,  fanout = 1, ratio = ratio) 
		
			#print(self.MODULES)
			#input("MODULE OK?")

		#skip read network don't need it
		#if("L1_OUT_READ_NETWORK" in net):
		#	NetworkComponent(valid_nets = net, self = self, unit_name = "L1_OUT_WRITE_NETWORK", loop_order = filter_loop_order(self.hc["LOOP_ORDER"], ["X", "Y", "N", "B"]), cast_skips = [], units = L1_OUT_BUF_BIT_LEN, bins = ['outputs_data'], prec = "PREC", data_obj = ['outputs_obj'], runtime_lambda = lambda p : 1, input_group = 1, unit_time_unrolled = 1, fanout = 1) 
			...
			if(ratio > 0): #meaning big to small
				config["L1_OUT_READ_NETWORK"].update({	
					"config": {
	...
	def prepare_data(self, params):
		...
		return {
			...
			"outputs_obj":
				...
				"name": "outputs",
				"file": o_file,
				"size": len(output_data.reshape((-1))),
				"indexing": "b*OUT*INPUT_X*INPUT_Y+n*INPUT_X*INPUT_Y + y*INPUT_X + x"	
			},
...
def core(test_id = 0, MODE = "TRACE", inputs = {}, benchmark = {}, 
	core_component = SystolicConv, core_name = "SystolicConv"):	
	...
	if (MODE == "DEBUG_OUR" or MODE == "OUR_DEBUG"):
		SIM_PARAMS['GEN_TRACE'] = True
		SIM_PARAMS['RUN_CPP'] = False
		SIM_PARAMS['save_np'] = False

	if ("DEBUG_OUR" in MODE  or "OUR_DEBUG" in  MODE ):
		SIM_PARAMS['GEN_TRACE'] = True
		SIM_PARAMS['RUN_CPP'] = False
		SIM_PARAMS['save_np'] = False
	...
	ld =  core_component(hardware_config)
...
#MODE: TRACE, TIME, GOLD, OUR, B1, B2	
def corev2(test_id = 0, MODE = "TRACE", inputs = {}, benchmark = {}):	
	...
	SIM_PARAMS['name'] = network_layer['layer_name']
	SIM_PARAMS["root"] = f"generated/Arch/SystolicConv/{design}"	
	SIM_PARAMS['name'] = SIM_PARAMS['name']+"_"+str(SIM_PARAMS["Wei_Sparsity"])+"_"+str(SIM_PARAMS['Act_Sparsity'])
	...
	#input("SIM_PARAMS OK?")
	#FLOW 1. compare against baselines and 
	ld =  SystolicConv(hardware_config)
	traces = ld.gen_perf_trace(network_layer,mapping, SIM_PARAMS) 
...
def run_fig6(base_hardware_config, base_mapping, base_network_layer, base_SIM_PARAMS, VALID_UNITS):
	...
	for lp in [ 
		...
		#	[1,16,  16,1,  1,  1,1],
			...
			if(unit in VALID_UNITS):
				for prec in [8,16 ]:
					for out_prec in [8,16,32]:
						for core_adder_type in ["RCAAdder2", "SimpleAdder2"]:
							for addern_type in ["AddTreeN", "SimpleAdderN"]:
								if(out_prec <= prec):
									continue
...
#, "L1_ACT_WRITE_NETWORK", "L1_ACT_READ_NETWORK"],	
	...
	VALID_UNITS = ["ADDER_TREE","OUT_LOADER", "PE_ARRAY", "WEI_LOADER", "ACT_LOADER","WEI_PE_CAST", "ACT_PE_CAST"]#[k  for k in  base_hardware_config] #["PE_ARRAY"]  #[,"WEI_LOADER"]#, "WEI_PE_CAST", "ACT_PE_CAST"]#[k  for k in  base_hardware_config]#["PE_ARRAY"]

	#VALID_UNITS = ["ADDER_TREE"]
	VALID_UNITS = VALID_UNITS[2]
