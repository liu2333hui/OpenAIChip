...
def gen_addr_cnt(hardware_config, meta_config, macro_config):
    ...
    for idx, flows in enumerate(CONV2D):
        ...
        #pres = ["L2_READ_", "L1_WRITE_", "L1_READ_","MAC_"]
        ...
        pres = ["L1_WRITE_", "L1_READ_", "MAC_", "ACC_"]
        ...
        s += "reg first;\n"
        s += "always@(posedge pe_array_last or negedge rst_n) begin\n\
                  if(~rst_n) begin\n\
                      first <= 0;\n\
                  end else begin \n\
                          first <= 1;\n\
        ...
        s += "wire counter_en = first? (addr_cnt_en_ & ~operation_done & ~L2_READ_stall)  :   ~addr_cnt_first & addr_cnt_en_;\n"
        ...
        s += "reg [31:0] acc_op_cnt;\n"

        s += "always@(*) begin\n"
        s += "    L2_READ_stall <=  ~first?  L2_READ_stall_to_mac | 0  : ~pe_array_last ;\n"
        s += "    L1_WRITE_stall <= ~first?   L2_READ_stall_to_mac  :  L1_WRITE_stall_to_mac | 0;\n"
        s += "    L1_READ_stall <=  ~first?   L1_WRITE_stall_to_mac  :  L1_READ_stall_to_mac | 0;\n"
        s += "    MAC_stall <=  ~first? L1_READ_stall_to_mac : MAC_stall_to_mac ;\n"
        s += "    ACC_stall =   0;\n"
        s += "end\n"
        ...
        s += "reg MAC_stall_to_mac;\n"
        
        #Due to mac unit being used
        s += "always@(posedge clk or negedge rst_n) begin\n\
                  if (~rst_n) begin \n\
        ...
        #There is an exceptional case, the first read
        s += "always@(posedge clk or negedge rst_n) begin\n\
                    if(~rst_n) begin\n\
        ...
        s += "reg  wei_L2_valid, act_L2_valid;\n"
        ...
        if("COUNTER" in hardware_config):
            if(hardware_config["COUNTER"].get("WEI_L1_GATE_SAME_READ", False)):
                s += "wei_L1_yi_addr = ~(wei_L1_prev_addr == wei_L1_buf_read_addr);\n"
            else:
                s += "act_L1_yi_addr = 1;\n"

            if(hardware_config["COUNTER"].get("ACT_L1_GATE_SAME_READ", False)):
                s += "act_L1_buf_read_en = ~(act_L1_prev_addr == act_L1_buf_read_addr);\n"
            else:
                s += "act_L1_yi_addr = 1;\n"
        ...
        s += "always@(*) begin\n\
                        ...
                        wei_L1_buf_read_en   <=wei_L1_yi_addr & (  ~first ? (L1_READ_en& ~L1_READ_stall) : pe_array_last);\n\
                        act_L1_buf_read_en   <=  wei_L1_yi_addr& (~first ? (L1_READ_en& ~L1_READ_stall) : pe_array_last);\n\
                  end\n"        
        ...
        if("TXX" in fd["LOOP"]):
            x_idx="(kkx_0+xx_0+xxx_0)"
        else:
            x_idx="(kkx_0+xx_0)"

        if("TYY" in fd["LOOP"]):
            y_idx="(kky_0+yy_0+yyy_0)"
        else:
            y_idx="(kky_0+yy_0)"
        ...
        s += "assign index_table_addr  = (bb_0)*ic*(x)*(y) + "+i_idx+"*(y)*(x)+"+x_idx+"*(y) + "+y_idx+";\n"
        ...
        s += "always@(negedge clk or negedge rst_n) begin\n\
            if(~rst_n) begin\n\
                loop_idx <= 0;\n\
            end else begin\n\
